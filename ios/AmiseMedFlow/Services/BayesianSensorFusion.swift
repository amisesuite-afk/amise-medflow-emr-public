...
enum BayesianSensorFusion {
    ...
    private static func fuseLabs(base: LabPanel, observations: [SensorObservation]) -> LabPanel {
        ...
        for obs in observations where obs.measurement != nil {
            ...
            case "ast", "asat":    lab.ast             = fuseDouble(lab.ast, fv, source: obs.source)
            ...
            case "calcium", "ca":  lab.calcium         = fuseDouble(lab.calcium, fv, source: obs.source)
            case "esr":            lab.esr             = fuseDouble(lab.esr, fv, source: obs.source)
