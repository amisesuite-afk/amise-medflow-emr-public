...
extension ClinicalScoringEngine {
    ...
    // MARK: - Manning Criteria (IBS)

    static func manning(_ i: ManningInput) -> ClinicalScore {
        let flags: [Bool] = [
            i.painRelievedByDefecation,
            i.looserStoolsWithOnsetOfPain,
            i.increasedFrequencyWithOnsetOfPain,
            i.abdomenVisiblyDistended,
            i.mucusPerRectum,
            i.feelingOfIncompleteEmptying
        ]
        let score = Double(flags.filter { $0 }.count)
        let items: [ScoredItem] = [
            .init(label: "Pain relieved by defecation",                      points: 1, present: i.painRelievedByDefecation),
            .init(label: "Looser stools with onset of pain",                 points: 1, present: i.looserStoolsWithOnsetOfPain),
            .init(label: "Increased stool frequency with onset of pain",     points: 1, present: i.increasedFrequencyWithOnsetOfPain),
            .init(label: "Abdomen visibly distended",                        points: 1, present: i.abdomenVisiblyDistended),
            .init(label: "Mucus per rectum",                                 points: 1, present: i.mucusPerRectum),
            .init(label: "Feeling of incomplete emptying",                   points: 1, present: i.feelingOfIncompleteEmptying),
        ]
        let (risk, interp, recs): (ScoreRisk, String, [String]) = switch Int(score) {
        case 0...1:
            (.low, "Manning 0–1 — IBS unlikely",
             ["Consider alternative diagnoses: IBD, coeliac disease, microscopic colitis",
              "Baseline bloods: FBC, CRP, TTG-IgA, faecal calprotectin",
              "Review dietary history; consider lactose / fructose intolerance"])
        case 2...3:
            (.moderate, "Manning 2–3 — IBS possible (sensitivity ~70%, specificity ~65%)",
             ["Positive predictive value improves with typical symptom chronicity (>6 months)",
              "Exclude red flags: rectal bleeding, nocturnal symptoms, weight loss, family history CRC",
              "Faecal calprotectin to exclude IBD (target <50 µg/g)",
              "Consider Rome IV criteria for formal IBS diagnosis"])
        default:
            (.high, "Manning ≥4 — IBS probable (sensitivity ~80%)",
             ["High Manning score supports clinical IBS diagnosis in absence of red flags",
              "No further investigation required in patients under 50 with no red flags",
              "Dietary advice: low-FODMAP trial for 4–6 weeks",
              "First-line therapy: antispasmodics (hyoscine, mebeverine) or peppermint oil",
              "Refer to gastroenterology if no response to 6-month conservative management"])
        }
        return ClinicalScore(
            ...
            recommendations: recs,
            items:          items,
            evidenceNote:   "Manning AP et al. BMJ 1978;2:653–654. Six symptoms differentiating IBS from organic colonic disease. ≥3 criteria increases IBS likelihood (LR+ ≈ 2.9). Best applied in patients without red-flag features; sensitivity 78%, specificity 72% at ≥3 criteria."
        )
    }
