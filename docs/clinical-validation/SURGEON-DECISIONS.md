...
| [approved-content-channel](changes/approved-content-channel.md#needs-sign-off) | Signed-off rule files without an app update (Migration 98): zebra rules and supplement catalogue items | Who may publish and withdraw (doctor and admin, or doctors only), a two-person rule, publishing only after the rule set's review is recorded, what may change without an app update, how fast a change or withdrawal reaches devices, offline devices, audit (7 items) |
