...
| 10 | Every step (row under the allergy status; sheet for the list) | `WhatsMissingPatient.result(for:bayes:)` → `WhatsMissing.whatsMissing` (`Services/WhatsMissingCore*.swift`, rules `Resources/WhatsMissingRules.json`) | latest vitals (NEWS2 parameters), labs (`latestLab`), weight / height, allergy state, pregnancy (`PlanSafetyFilter.signals`), supplements answer, prescriptions (today's = planned), suggested / ordered investigations, `DiagnosisScoreMapper` + stored scores, `DecisionSupportPatient.input` (probed with adverse values), `SuspectedCancerScreening.prompt` (ferritin check), `DiagnosticReasoningAdapter.report` (first discriminator) | Ranked items: what, why, one action (step / tool, or a suggested investigation) (`ios.missing`) | "What's missing" row + sheet (`WhatsMissingRow.swift`); the populators also read the record fill (`PatientScoreAutoPopulator+RecordFill.swift`) |
