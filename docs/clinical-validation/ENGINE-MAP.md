...
| Assessment diagnostic reasoning | `buildDiagnosticReasoning` (`lib/diagnostic-reasoning.ts`) → `@workspace/triage-engine/diagnostic-reasoning` (explain, `informationGain` over the top 3, closure guard, time-out, zebras, longitudinal) | PANE state and likelihoods, locked working diagnosis, vital records (NEWS2 series), record text, supplement history, earlier encounters and imported labs | for / against / missing / doesn't fit, best next discriminator, closure alerts, time-out, zebras, longitudinal patterns (`web.reasoning`) | "Diagnostic reasoning" card under the PANE differential (Alt+R) |
