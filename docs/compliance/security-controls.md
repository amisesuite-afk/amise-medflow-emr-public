...
| Row-level security enabled on core tables; policy presence enforced in CI | `supabase-schema.sql:299-307`; `scripts/src/lint-rls-policies.ts` (`REQUIRED_POLICIES`, lines 41-63) | This is a **presence** check, not an enforcement test (`CLAUDE.md`) |
