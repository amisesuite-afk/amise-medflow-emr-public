...
| | |
|---|---|
| Status | Draft v0.2, 2026-09-25 (v0.1: 2026-09-24). Code baseline `c9a7293`, refreshed against `cc83845` and `23904fd`. **No vendor has been added or removed since v0.1, and no agreement has been recorded** |
...
| 6 | **Meta Platforms (WhatsApp Cloud API)** | Alternative WhatsApp provider: inbound webhook and outbound replies (`routes/whatsapp.ts`) | Phone numbers, message content | Unknown | **Required** if enabled. A BAA is generally not offered; **consider not using it for PHI** | Not yet signed / unknown. Active only if `WHATSAPP_ACCESS_TOKEN` is set, **to confirm**. Sends are now MODE-gated (`lib/whatsapp-send.ts`, `e094063`) |
| 7 | **Telnyx** | Alternative WhatsApp/SMS provider (`routes/whatsapp.ts:268-286`) | Phone numbers, message content | Unknown | **Required** if enabled | Not yet signed / unknown. Active only if `TELNYX_API_KEY` is set, **to confirm**. Sends are now MODE-gated (`lib/whatsapp-send.ts`, `e094063`) |
...
| 13 | **Apple** | iOS platform. Speech recognition for dictation, **on-device whenever the recogniser supports it** since `16435f3`, server-based otherwise (`SpeechService.swift`). iCloud device backup (may include the local store). Calendar sync of EventKit events carrying patient names. App distribution | Dictated clinical speech, only where on-device recognition is unavailable. Potentially the whole local store via device backup. Patient names and procedures in calendar | Unknown | Recommended. Apple does not generally sign BAAs for these consumer services, so **prefer on-device recognition and exclude the store from backup** | Not yet signed / unknown |
...
### Status of these actions (2026-09-25)

| # | Status |
|---|---|
| 1 | Open. No agreement recorded |
| 2 | Open (practice owner). The technical switch-off is ready: `DISABLE_AI=true` on Render **and** the front-desk Vercel project stops every Anthropic and Whisper call (`113b1d9`). Whether it is set in production is **to confirm** |
| 3 | Done (`f430146`) |
| 4 | Open. Web and API Sentry still have no `beforeSend` |
| 5 | Open |
| 6 | Open. The new CI lint `lint:no-external-qr` covers QR and chart-image services only. The multi-tenancy plan (`docs/MULTI-TENANCY-PLAN.md`) notes that each customer practice will need its own register, or this one extended with per-practice accounts |

No new subprocessor was introduced by the changes since v0.1. The fresh-database migration check runs in CI on an in-process PGlite database with no patient data (`bf6f602`), and the `qrcode` package runs in the browser.
