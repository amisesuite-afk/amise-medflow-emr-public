...
| 14 | **goQR.me (api.qrserver.com)** | QR-code image generation in the dashboard (`QuestionnaireManagerTab.tsx:720`) | **The patient questionnaire URL, including its session token** | Unknown (EU company, to confirm) | Should not receive data. **Replace with local QR generation** | No agreement. Remove |
...
2. **Decide on Anthropic (9) and OpenAI (10).** Either sign a BAA/DPA with zero retention, or switch them off in production by unsetting `ANTHROPIC_API_KEY` (Render and front-desk Vercel) and `OPENAI_API_KEY`. Separately, make `DISABLE_AI` cover every AI route (an engineering task).
3. **Remove the api.qrserver.com call (14).** It leaks a patient session token to a third party with no agreement.
