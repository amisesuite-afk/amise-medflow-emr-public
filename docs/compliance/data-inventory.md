...
- **API server.** It connects as `service_role` and bypasses RLS. Its route gate, `requireStaffAuth()` (`artifacts/api-server/src/lib/supabase.ts:28-50`), accepts **any** valid Supabase user JWT, or `x-staff-token` equal to `CRON_SECRET`. Only one route checks the caller's role on the server: `routes/visit-lifecycle.ts:266-271`, which requires `doctor`.
...
| 16 | **api.qrserver.com (goQR.me)** ✅ | The dashboard renders a QR code by sending the patient's **questionnaire URL, including its session token**, to this third party | Staff open the questionnaire manager | `artifacts/dashboard/src/pages/tabs/QuestionnaireManagerTab.tsx:200, 720` |
