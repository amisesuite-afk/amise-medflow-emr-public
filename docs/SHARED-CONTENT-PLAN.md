...
| Diagnostic-reasoning thresholds, probe-cost terms, time-out checklist, longitudinal thresholds, other-specimen terms (`diagnostic-reasoning-rules`) | `diagnostic-reasoning/core.ts`, `longitudinal.ts` | `DiagnosticReasoningCore.swift`, `LongitudinalPatterns.swift`, `ZebraCheck.swift` | `diagnostic-reasoning-parity.test.ts` (regex over Swift) + shared vectors `DiagnosticReasoningVectors.json` | Vectors only | 707 / 676 (mostly logic) | P1 | Low-medium | 2 |
