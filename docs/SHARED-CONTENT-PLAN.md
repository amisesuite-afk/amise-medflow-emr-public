...
| Treatment-decision table: score / result actions, effects, harms, modifiers (`treatment-decision-support`) | `pane-engine/src/decision/treatment-decisions.json` → **`clinical-content/rules/treatment-decisions.json`** | `Resources/TreatmentDecisions.json` (deleted) | Was: `decision-content-parity.test.ts` byte-identical copies, generated vectors, Swift key presence. Now: one file, `lint:shared-content` typed schema check, generated vectors | **Yes — one file** | 3,069 (JSON) | P1 | None (one file) | **2 — done (2026-09-26)** |
| What's-missing rules (`whats-missing`) | `pane-engine/src/whats-missing/whats-missing-rules.json` → **`clinical-content/rules/whats-missing-rules.json`** | `Resources/WhatsMissingRules.json` (deleted) | Was: `whats-missing-parity.test.ts` byte-identical copies, generated vectors, Swift key presence. Now: one file, `lint:shared-content` typed schema check, generated vectors, Swift core key presence | **Yes — one file** | 161 (JSON) | P1 | None (one file) | **2 — done (2026-09-26)** |
...
- A rule set with more than one shared file (phase 2: `lifestyle-practices` with the questionnaire
  wording `lifestyle-questions.json`): the extra file carries the same `id` and `version`, the
  registry stamp stays on the main file, and the pair is listed in `COMPANION_FILES` in
  `shared-content.ts`.
- A Swift value decoded by hand from another JSON shape (`TreatmentDecisions.Triple` from
  `[low, point, high]`, `WhatsMissing.ProbeValue` from a number or a word) is declared in the
  mapping's `customDecoded` with the JSON types it accepts; keep every other struct plain.
- iOS never guesses when a file is missing: the engine shows nothing, and a caller that would
  otherwise make a call from empty data checks the engine's `isAvailable` (visit continuity,
  lifestyle questions).
