...
| Measure | Count |
|---|---|
| Rule sets registered | **44**, in about 220 files (45 with `treatment-decision-support`, added 2026-09-25, and more since; `whats-missing` and `history-frames` added 2026-09-26) |
...
| `history-frames` (P1, added 2026-09-26) | `lib/triage-engine/src/history-frames/{types,frames,classify,index,record-only}.ts`, `ios/AmiseMedFlow/Services/{HistoryFrames,HistoryFrameData}.swift` (generated twin), `Views/Consultation/ConsultationView+HistoryFrame.swift`, `artifacts/dashboard/src/lib/hpi-fields.ts` | TypeScript data + generated Swift twin | `HISTORY_FRAMES_VERSION = '1.0.0'` | Registry entry (BTS 2006 / ERS 2020 cough, MRC dyspnoea, NICE NG12, Macleod's) | Complaint (+ system) -> symptom type -> history frame: 41 frames, 21 types (SOCRATES for pain only, with regional sites). Every chip maps to an existing engine feature or is listed as record only; no weights. Shared vectors `ios/AmiseMedFlowTests/Resources/HistoryFrameVectors.json`; `lint:history-frames`; change log `docs/clinical-validation/changes/history-by-complaint.md` |
