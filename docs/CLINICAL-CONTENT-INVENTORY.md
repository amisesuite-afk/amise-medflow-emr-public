...
| `pane-engine-disease-model` (P1) | `lib/pane-engine/src/vademecum/specialties/*.ts` (13 files), `_other.ts`, `registry.ts`, `index.ts`, `engine/{bayes,infoGain,modifiers}.ts`, `constants.ts` | TypeScript objects `DiseaseNode{id, label, icd10, prior, features{featureId → P(feature\|disease)}}` | None | **None** | 130 diseases, 135 features, 58 prior modifiers. C-8 |
...
| `ios-diagnosis-radiation-engine` (P2) | `DiagnosisRadiationEngine.swift`, `+DiseaseDictionary*.swift` (8 files) | Swift dictionaries: investigations, plan template, billing codes, consent category, score variables | None | Guideline names in many rationale strings (NICE 33, ESC, AHA, IDSA, ACG, …), mostly without years | Presented, never auto-applied |
...
| Registry id | Location | Format | Version today | Citations | Notes |
|---|---|---|---|---|---|
| `triage-rules-red-flags` (P1) | `lib/triage-engine/src/rules.ts` | TypeScript: `RED_FLAGS` (regex patterns), slot rules, exclusions, public holidays, pathway definitions, reminder cascade, `FORBIDDEN_PATTERNS` | **`RULES_VERSION = '1.2.0'`**, not recorded with results | None | Hazard H-01: no sensitivity evidence |
...
| `adaptive-triage` (P1) | `lib/triage-engine/src/adaptive-triage.ts` | TypeScript rules | None | Some | Inline vital-sign thresholds (for example HR > 120, SpO₂ < 94) |
| `cancer-screening` (P2) | `lib/triage-engine/src/cancer-screening.ts` | TypeScript criteria `{rule, met, guideline}` | None | Per criterion: NICE NG12, BSG/ACG. **No NG12 version** | |
