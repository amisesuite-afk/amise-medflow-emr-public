...
| `pane-engine-management-protocols` (P1) | `lib/pane-engine/src/management/protocols/*.ts` (15 files), `index.ts`, `types.ts` | TypeScript `ManagementProtocol{kind, guidelines, keyPoints, redFlags, investigations[{onlyIf?}], management[{phase, step, onlyIf?}], medications[{drugName, dose, frequency, route, onlyIf?}], allergyAlternatives}`; `MANAGEMENT_PROTOCOLS_VERSION` | 1.0.0 | **Per protocol** (`guidelines` field; guideline and year in the step text) | 196 protocols (incl. medical, obstetric and paediatric emergencies — recognise, first actions, redirect), 747 medication entries. Longest-prefix ICD lookup, `resolveProtocol`, PANE-id aliases |
| `plan-safety-filters` (P1) | `lib/pane-engine/src/management/planSafety.ts`, `artifacts/dashboard/src/lib/plan-builder.ts` | TypeScript; `PLAN_SAFETY_VERSION` | 1.0.0 | **Per rule** (BNF/BNFc, RCOG GTG 37a/b, NICE NG89, BRIDGE, PAUSE, BSG/ESGE 2021, ESC/ESAIC 2022, CPOC 2021, …) | Adapts a protocol to the patient on record: allergy classes, pregnancy, under-16 dosing, VTE, anticoagulation, SGLT2, steroid cover, anaesthetic hazards, frailty. Clinician-facing only |
...
| `supplement-catalogue` (P2) | `artifacts/dashboard/src/lib/supplement-catalogue.ts`, `supplement-prompts.ts`; `ios/AmiseMedFlow/Services/SupplementCatalogue.swift`, `SupplementAlerts.swift` | Herbs / supplements catalogue (names, concern, stop time, harms, source), shared patient question and herbal pre-op text, prompt texts and trigger words | `SUPPLEMENT_CATALOGUE_VERSION` | Per item (owner's briefing; Ang-Lee JAMA 2001; SPAQI 2025; DILIN; LiverTox; Saper 2008; Nortier 2000) | 17 items. `lint:interaction-parity` compares every field. Herbal pre-op patient text approved 2026-09-25; catalogue and thresholds not clinically reviewed |
...
| `lifestyle-practices` (P2) | `lib/triage-engine/src/lifestyle-practices.ts`, `lifestyle-questions.ts`; `ios/AmiseMedFlow/Services/LifestylePractices.swift`, `LifestylePractices+Patient.swift`; adapter `artifacts/dashboard/src/lib/lifestyle-context-web.ts` | TypeScript and Swift twins: lifestyle record (fasting, complementary therapies, night shift, sleep), safety prompts, evidence-graded non-drug suggestions; APCQ lifestyle questions | **`LIFESTYLE_PRACTICES_VERSION = '0.1.0'`** | Per rule (briefing §10 references; IDF-DAR 2021 named in §4) | From the practice owner's evidence briefing (Sept 2026). Shared vectors in `lifestyle-practices.test.ts` / `LifestylePracticesTests.swift`. Sign-off list: `docs/clinical-validation/changes/lifestyle-practices.md` |
