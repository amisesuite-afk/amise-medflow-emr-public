...
export default function PlanTab() {
  ...
  // Tracks the last auto-generated plan text so we can detect user edits
  const autoGeneratedPlanRef = useRef<string>('');
  ...
  // clinical decision — it shouldn't require the working diagnosis to be
  // separately "locked" before the plan reflects it. Regenerates immediately
  // using the just-picked variant (not the not-yet-updated selectedVariant
  // state) unless the clinician has already customised the plan text, in
  // which case we only update the grade selection/urgency banner and leave
  // their edits alone — "Generate plan" remains available to force it.
