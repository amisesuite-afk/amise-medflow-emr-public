...
export default function AmbientConsultation({ visitType, onDetailedMode, onFinalise, compact = false }: Props) {
  ...
  // Exam systems start empty ("not examined"). Normal text is applied only by the clinician's
  // own "◎ Nml" tap on a system — never on entering the exam phase (UX review C2). This effect
  // used to write normal General/Abdomen/CVS/Resp/Extremities/Wound text into the record.
        ...
        {phases.map((p, i) => {
          ...
          return (
            <button
              ...
              className={`phase-nav__btn${active ? ' phase-nav__btn--active' : done ? ' phase-nav__btn--done' : ''}`}
              style={{ opacity: i > phaseIdx ? 0.65 : 1 }}
            >
              {done && <span className="phase-nav__check">✓</span>}
              {p.label}
              {i < phases.length - 1 && (
                <span className="phase-nav__sep">›</span>
              )}
            </button>
          );
        })}
