...
interface TheatreCase {
  id: string; patientName: string; procedure: string; durationMin: number;
  position: string; anaesthetic: string; equipment: string[];
  antibiotics: string; notes: string; group: ProcGroup;
}
...
const SESSIONS: Record<SessionKey, { label: string; startTime: string; durationMin: number }> = {
  morning:   { label: 'Morning list',   startTime: '08:30', durationMin: 270 },
  afternoon: { label: 'Afternoon list', startTime: '13:30', durationMin: 210 },
  full_day:  { label: 'Full day',       startTime: '08:30', durationMin: 480 },
  ercp:      { label: 'ERCP / Endo',    startTime: '08:30', durationMin: 240 },
};
...
function displayDate(s: string): string {
  ...
  return new Date(y, m - 1, d).toLocaleDateString('en-LC', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
}

function turnover(g: ProcGroup) { return g === 'endoscopy' ? 10 : 15; }
function uid() { return Math.random().toString(36).slice(2, 8); }

const EMPTY_ADD = { patientName: '', procedure: '', durationMin: '', notes: '', position: '', anaesthetic: '', equipment: [] as string[], antibiotics: '', group: 'other' as ProcGroup };
...
export default function TheatreListBuilderCard() {
  ...
  const totalTurnover = cases.length > 1
    ? cases.slice(0, -1).reduce((s, c) => s + turnover(c.group), 0) : 0;
  ...
  function handleAddCase() {
    ...
    setCases(prev => [...prev, { id: uid(), patientName: add.patientName.trim(), procedure: add.procedure.trim(), durationMin: dur, position: add.position, anaesthetic: add.anaesthetic, equipment: [...add.equipment], antibiotics: add.antibiotics, notes: add.notes.trim(), group: add.group }]);
    setAdd({ ...EMPTY_ADD });
    setShowAdd(false);
  }

  function moveUp(i: number) { setCases(p => { const a = [...p]; [a[i-1], a[i]] = [a[i], a[i-1]]; return a; }); }
  function moveDown(i: number) { setCases(p => { const a = [...p]; [a[i], a[i+1]] = [a[i+1], a[i]]; return a; }); }
  function remove(i: number) { setCases(p => p.filter((_, j) => j !== i)); }
  ...
  function printList() {
    ...
    const caseRows = cases.map((c, i) => {
      const s = schedule[i];
      const noAbx = c.antibiotics === 'None' || c.antibiotics === 'None routine';
    ...
    const finish = schedule.length ? schedule[schedule.length - 1].end : sessionCfg.startTime;
    w.document.write(`<!DOCTYPE html><html><head><title>Theatre List — ${displayDate(date)}</title>
    <style>
    ...
    <div class="sub">AMISE MEDICAL SERVICES · ${loc}</div>
    <div class="meta">
      <div><strong>Date:</strong> ${displayDate(date)}</div>
      <div><strong>Session:</strong> ${sessionCfg.label} (${sessionCfg.startTime}–${addMins(sessionCfg.startTime, capacity)})</div>
  ...
  return (
    <CollapsibleCard title="Theatre Operating List Builder" defaultOpen={false}>
      <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>
        Compose the day's operating list from procedure templates. Auto-calculates start times, block capacity, and equipment needs. Print a formatted list for the anaesthetist and scrub team.
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', marginBottom: 3 }}>Session</div>
          <select value={session} onChange={e => setSession(e.target.value as SessionKey)}
            ...
            {(Object.entries(SESSIONS) as [SessionKey, typeof SESSIONS[SessionKey]][]).map(([k, v]) => (
              <option key={k} value={k}>{v.label} ({v.durationMin} min)</option>
            ))}
          </select>
        </div>
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', marginBottom: 3 }}>Location</div>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 11 }}>
          <span style={{ fontWeight: 600, color: '#374151' }}>Block capacity — {sessionCfg.label}</span>
        ...
        <div style={{ height: 10, borderRadius: 5, background: '#e5e7eb', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${pct}%`, borderRadius: 5, background: pctColor, transition: 'width .3s' }} />
        </div>
        <div style={{ display: 'flex', gap: 14, marginTop: 4, fontSize: 10, color: '#9ca3af' }}>
      ...
      {/* Case list */}
      {cases.length === 0 && (
        <div style={{ textAlign: 'center', padding: '18px 0', color: '#9ca3af', fontSize: 12, background: '#f9fafb', borderRadius: 8, marginBottom: 10 }}>
          No cases yet — click <strong>Add Case</strong> to start building the list.
        </div>
      )}
      ...
      {cases.map((c, i) => {
        const s = schedule[i];
        return (
          <div key={c.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, marginBottom: 8, overflow: 'hidden' }}>
            <div style={{ background: '#1e3a2e', color: '#fff', padding: '6px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 12, fontWeight: 700 }}>Case {i + 1} · {s.start}–{s.end} ({c.durationMin} min)</span>
              <div style={{ display: 'flex', gap: 4 }}>
                {(['▲', '▼'] as const).map((dir, di) => (
                  <button key={dir} onClick={() => di === 0 ? moveUp(i) : moveDown(i)}
                    disabled={di === 0 ? i === 0 : i === cases.length - 1}
                    style={{ background: 'rgba(255,255,255,.15)', border: 'none', color: '#fff', borderRadius: 4, padding: '2px 7px', fontSize: 11, cursor: 'pointer', opacity: (di === 0 ? i === 0 : i === cases.length - 1) ? .3 : 1 }}>
                    {dir}
                  </button>
                ))}
            ...
            <div style={{ padding: '8px 12px', fontSize: 12 }}>
              ...
              {c.equipment.length > 0 && <div style={{ color: '#374151', fontSize: 11, marginTop: 2 }}>Equipment: {c.equipment.join(', ')}</div>}
      ...
      {showAdd && (
        <div style={{ border: '1px solid #d1d5db', borderRadius: 8, padding: 14, marginBottom: 10, background: '#f9fafb' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleAddCase} disabled={!add.patientName.trim() || !add.procedure.trim()}
      ...
      {/* Action bar */}
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: cases.length > 0 ? 4 : 0 }}>
        {!showAdd && (
        ...
        {cases.length > 0 && (
          <>
            ...
            <button onClick={() => setCases([])}
              style={{ padding: '8px 14px', borderRadius: 7, fontSize: 12, fontWeight: 600, background: '#fef2f2', color: '#991b1b', border: '1px solid #fca5a5', cursor: 'pointer' }}>
              Clear list
            </button>
          </>
        )}
