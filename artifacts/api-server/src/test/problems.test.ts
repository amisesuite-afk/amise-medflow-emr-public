...
describe('GET /api/problems/:patientId', () => {
  it('returns empty array when no problems', async () => {
    ...
    const res = await request(app).get('/api/problems/pat-1');
  ...
  it('returns active problems', async () => {
    const rows = [
      { id: 'prob-1', patient_id: 'pat-1', encounter_id: null, condition: 'Hypertension', status: 'active', created_at: '2026-07-01T00:00:00Z', updated_at: '2026-07-01T00:00:00Z' },
      { id: 'prob-2', patient_id: 'pat-1', encounter_id: null, condition: 'T2DM', status: 'active', created_at: '2026-07-02T00:00:00Z', updated_at: '2026-07-02T00:00:00Z' },
    ];
  ...
  it('filters by ?status=resolved', async () => {
    mockFrom.mockReturnValueOnce(mkChain(ok([])));
    const res = await request(app).get('/api/problems/pat-1?status=resolved');
    expect(res.status).toBe(200);
  ...
  it('ignores invalid status filter', async () => {
    mockFrom.mockReturnValueOnce(mkChain(ok([])));
    const res = await request(app).get('/api/problems/pat-1?status=invalid');
    expect(res.status).toBe(200);
    const chain = mockFrom.mock.results[0].value;
...
describe('POST /api/problems', () => {
  ...
  it('returns 400 when condition is missing', async () => {
    const res = await request(app).post('/api/problems').send({ patientId: 'pat-1' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/condition/i);
  });

  it('returns 400 when condition is blank', async () => {
    const res = await request(app).post('/api/problems').send({ patientId: 'pat-1', condition: '   ' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/condition/i);
  });

  it('creates a problem and returns 201', async () => {
    const created = { id: 'prob-1', condition: 'Hypertension', status: 'active', created_at: '2026-07-31T00:00:00Z' };
    mockFrom.mockReturnValueOnce(mkChain(ok(created)));
    const res = await request(app).post('/api/problems').send({ patientId: 'pat-1', condition: 'Hypertension' });
    expect(res.status).toBe(201);
    expect(res.body.problem.id).toBe('prob-1');
    expect(res.body.problem.status).toBe('active');
  });

  it('upserts on conflict (patient_id,condition)', async () => {
    const created = { id: 'prob-1', condition: 'T2DM', status: 'active', created_at: '2026-07-31T00:00:00Z' };
    mockFrom.mockReturnValueOnce(mkChain(ok(created)));
    const res = await request(app).post('/api/problems').send({ patientId: 'pat-1', condition: 'T2DM', encounterId: 'enc-1' });
    expect(res.status).toBe(201);
    const chain = mockFrom.mock.results[0].value;
...
describe('PATCH /api/problems/:id', () => {
  ...
  it('returns 404 when problem not found', async () => {
    ...
    const res = await request(app).patch('/api/problems/prob-1').send({ status: 'resolved' });
  ...
  it('updates condition text', async () => {
    mockFrom
      .mockReturnValueOnce(mkChain(ok({ id: 'prob-1' })))
      .mockReturnValueOnce(mkChain(ok(null)));
    const res = await request(app).patch('/api/problems/prob-1').send({ condition: 'Essential hypertension' });
    expect(res.status).toBe(200);
  ...
  it('returns 502 on DB update error', async () => {
    ...
    const res = await request(app).patch('/api/problems/prob-1').send({ status: 'active' });
    expect(res.status).toBe(502);
  });
});
