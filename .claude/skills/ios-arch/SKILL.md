...
| `BayesianDiagnosisEngine` | Differential diagnosis. `DiagnosticDatabase.json` (v2.2.0, 29 presentations; exam-sign and decision-rule LRs come from the shared `clinical-content/rules/*.json`) decodes; the built-in lists (`+*Candidates.swift`) are the fallback if it ever fails. Settings → Diagnostics shows which one (`DiagnosticDatabaseInfo`). `BayesianDecisionEngine` = max-rule/utility decisions on top. Registry and plan: `clinical-content/registry.json`, `docs/CLINICAL-CONTENT-UPGRADES.md` |
