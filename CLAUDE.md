...
| `LAB_FEED_SECRETS` | Laboratory results feed (`POST /api/lab-feed/inbound`, `docs/LAB-FEED.md`): one secret per laboratory, `labid:secret` pairs separated by commas (`slulab:<64 hex>,…`; `=` also accepted). The laboratory sends `x-lab-id` + `x-lab-feed-key`; compared in constant time before the body is read. Lab ids `[a-z0-9_-]`, secrets ≥ 32 characters (shorter entries are ignored). Unset or empty = feed off (503). Render only; never in the browser |
| `DOCTOR_NOTIFY_EMAIL` | Email for escalations and daily summary. Also receives the lab feed's critical-result alert (staff-only, no patient data, MODE-gated) |
...
- **Clinical rules live in `clinical-content/rules/*.json`; change the JSON, not a platform copy.** Web and iOS read the same file: the web imports it (`resolveJsonModule`; the TS modules keep their old export names), iOS bundles the whole folder as the `rules` folder reference in `ios/project.yml` and decodes it with Codable structs through `Services/SharedClinicalContent.swift` (a missing or undecodable file shows nothing, never crashes, and is listed under Settings → Diagnostics → "Shared clinical rules"). Each file has a JSON Schema in `clinical-content/schemas/`; `pnpm --filter @workspace/scripts run lint:shared-content` (CI) fails when a file does not validate, when its `id`/`version` do not match its registry entry (json-key version stamp), when the Swift Codable structs or TS interfaces that read it disagree with the schema (field names, optionality, types, string-enum values; read from source), or when `SharedClinicalContent.File` / `project.yml` does not include it. Shared so far: `zebra-rules.json`, `supplement-catalogue.json` (incl. the approved herbal patient paragraph), `lifestyle-practices.json` (content only; the rule logic is still twinned with hand-ported vectors) with its companion `lifestyle-questions.json` (the patient questionnaire wording, same rule set and version, also under `lint:patient-instructions`), `exam-signs.json`, `decision-rules.json`, `diagnostic-reasoning-rules.json` (iOS compiles its `thresholds` in, pinned by the parity test), `treatment-decisions.json` (decision support; iOS `TreatmentDecisions.Triple` is decoded by hand from `[low, point, high]`, declared as `customDecoded` in the lint), `whats-missing-rules.json`, `visit-continuity.json` (word rules only; the matching logic is twinned with shared vectors `VisitContinuityVectors.json`), `negation-cues.json` (the NegEx-style cue lists of `negation.ts` / `NegationMatcher.swift`; the rule stays twinned; on iOS a missing file negates nothing). Drug interactions are not shared yet (own lint, deferred). A new field goes into the schema, the Swift struct and the TS interface in the same PR; a content change bumps the JSON `version` with the registry entry. Survey of the other twins and the migration order: `docs/SHARED-CONTENT-PLAN.md`.
...
- **iOS `DiagnosticDatabase.json` (2.3.0) decodes and is live content awaiting sign-off.** Before 2.0.0 it failed `CandidateSpec` decoding (non-integer `logLR`/`logPrior`) and the app silently ran the built-in fallback lists. 2.0.0 and later (162+ pools, 1,350+ candidates, 29 complaint `presentations`) decode, and `lint:guideline-registry` now fails if it would not (`scripts/src/diagnostic-database-schema.ts` mirrors the Swift Codable structs; change both together). Routing reads the chief complaint only; displayed percentages divide stored log units by 5; places 4–5 of the five shown are held for the likeliest emergency/critical diagnoses. Settings → Diagnostics shows "Using the database" or "Built-in fallback lists" (`DiagnosticDatabaseInfo.swift`). Weights, priors and citations are listed for surgeon/physician sign-off in `docs/clinical-validation/changes/fix-ios-differential.md` `ios-last-criticals.md` (2.1.0: NEWS2 boost by urgency tier, post-op day, masking contexts) and `evidence-exam.md` (2.2.0: exam-sign and decision-rule likelihood ratios from `clinical-content/rules/exam-signs.json` / `decision-rules.json`) and `ios-outcomes-calculators.md` (2.3.0: the bands of the rules that gained an iOS calculator); don't put a `lastReviewed` date on it without that review. The `rule` / `sign` features are generated (`scripts/src/gen-exam-evidence-db.ts`, checked by `exam-evidence-db.test.ts`): change the shared JSON and regenerate, never hand-edit them. A decision rule linked to an iOS calculator (`ios.param` / `ios.activeScore` in `decision-rules.json`) needs the ActiveScore case, the Patient field, the `infer` parameter and its `ruleValues` line (`decision-rules-ios-links.test.ts` checks them).
...
- **Auth model is single-tenant, role-based — not per-patient.** Access control runs on `user_profiles.role` (`front_desk`/`nurse`/`doctor`/`admin`, via the `auth_role()` SQL function), not row ownership. Once Migration 89 (`supabase-staff-only-rls-migration.sql`, written but not yet applied) runs:
  ...
  - `patients` UPDATE has column guards (BEFORE UPDATE triggers). Front desk may change admin and patient-reported intake columns only, and a portal patient only their own contact fields. Anything else fails with `42501`. Adding a `patients` column means deciding whether to add it to either allow-list, and keeping iOS `FrontDeskPatientColumns` in step. The front-desk scheduling columns include `visit_type` and `appointment_type` (the booking type; the column itself is added by Migration 97, and iOS syncs it in its own request, `SyncService+AppointmentType.swift`). The service role (API server) bypasses both guards.
...
- **The diagnostic-reasoning layer is a web/iOS twin with shared vectors and one shared rule file.** Web: `lib/triage-engine/src/diagnostic-reasoning/*` (core, `families.ts` + `adapter-rules.ts` — diagnosis families, working-diagnosis label matching, two conditions, already named, contradiction LR ≤ 0.2 —, zebra rule set, longitudinal patterns) + `artifacts/dashboard/src/lib/diagnostic-reasoning.ts` (PANE adapter) → `DiagnosticReasoningPanel` on the Assessment step. iOS: `DiagnosticReasoningCore.swift`, `DiagnosticReasoningRules.swift` (twin of families / adapter rules), `ZebraCheck.swift`, `LongitudinalPatterns.swift`, `DiagnosticReasoningAdapter.swift` → `DiagnosticReasoningCard.swift` on the Diagnosis step. Thresholds and rule vocabulary live in `clinical-content/rules/diagnostic-reasoning-rules.json` (iOS compiles the core thresholds in; `scripts/src/diagnostic-reasoning-parity.test.ts` pins them, the cost terms and checklist); zebra rules in `zebra-rules.json`. Both run `ios/AmiseMedFlowTests/Resources/DiagnosticReasoningVectors.json`. The iOS adapter reads the database's stated `likelihoodRatio` (a stored logLR 2 is LR 1.5, not exp(2/5) = 1.49), looks the working diagnosis up among every scored candidate (`DiagnosisResult.rankedBelow` on the first result), and treats one evidence label as one finding. iOS database terms and the text parser read clauses (`RecordClauses.swift`, twin of `lib/triage-engine/src/record-clauses.ts`, shared `RecordClauseVectors.json`: negated lists, family history, lay guesses, "?queries", remedies that did not help); iOS does not apply the web mapper's "not current" rule (database terms mix current findings and history). Deterministic, reads the engines' own evidence (never changes a weight), adds nothing to the record without a tap. Registered as `diagnostic-reasoning-rules` (shared), `diagnostic-reasoning-web` (how the web reads PANE evidence) and `diagnostic-reasoning-zebras` (unreviewed; sign-off list in `docs/clinical-validation/changes/diagnostic-reasoning.md`).
- **The "What's missing" strip is a web/iOS twin with shared vectors.** Core `lib/pane-engine/src/whats-missing/*` and `ios/AmiseMedFlow/Services/WhatsMissingCore*.swift`, rules in the one shared file `clinical-content/rules/whats-missing-rules.json` (web `rules.ts`, iOS `WhatsMissingRules.swift` through `SharedClinicalContent`; checked by `lint:shared-content`); adapters `artifacts/dashboard/src/lib/whats-missing-web.ts` (→ `WhatsMissingStrip` under the patient header) and `WhatsMissingPatient.swift` (→ `WhatsMissingRow` under the allergy status). `scripts/src/whats-missing-parity.test.ts` fails when the web core does not read the shared file, a retired copy comes back, or the vectors are stale (`pnpm --filter @workspace/scripts run gen:whats-missing-vectors`). Ranking is lexicographic: safety basics → decision (adverse-probe band flip, risk score, NG12 ferritin, discriminator while unconfirmed) → score completeness. The same record fill pre-fills the Scales-step calculators ("from record") and the iOS populators (`mergeRecord`). A new score input or safety basic goes in the shared rules JSON (and its schema), both cores and the vectors in the same PR. Nothing is ordered or recorded without a tap. Registered as `whats-missing` (unreviewed; sign-off list in `docs/clinical-validation/changes/whats-missing.md`).
- **The history questions follow the chief complaint (history frames, web/iOS twin).** `lib/triage-engine/src/history-frames/` (`@workspace/triage-engine/history-frames`) classifies the complaint (+ iPad specialty group) into a symptom type and frame — SOCRATES for pain only; cough, dyspnoea, lump, bleeding, bowel, dysphagia … otherwise. iOS reads the generated twin `ios/AmiseMedFlow/Services/HistoryFrameData.swift` (never edit it by hand: change `frames.ts`/`classify.ts`, then `pnpm --filter @workspace/scripts run gen:history-frames` and `-- --record-only`); the web card and CC strip use `artifacts/dashboard/src/lib/hpi-fields.ts`, and frame answers reach PANE through `FRAME_KEY_RULES` in `socrates-to-features.ts`. `lint:history-frames` fails on a stale twin, a complaint vector that changed frame (`ios/AmiseMedFlowTests/Resources/HistoryFrameVectors.json`), radiation outside pain, a site outside its region, contradictory chips, or a chip that neither reaches an engine feature nor is listed record only. Rename a chip with an alias so stored answers still render. On iOS a chip carries its question into the engine (`engine-dimensions.ts` → generated `chipLabels`/`chipRecordFields`, `BayesianDiagnosisEngine+ChipDimensions.swift`): the finding features read "aggravating: Coughing", and chips under `exam`/`pmh`/`history`/`pshx`/`social`/`inv` are also read as that record field. Database finding terms of ≤ 4 letters match whole words (`FeatureTerm.stems` excepted, `BayesianDiagnosisEngine+FeatureTerms.swift`, mirrored by the audit), and a cough mention is classified as symptom / aggravating factor / sign by `cough-mention.ts` (twin `CoughMention.swift`, shared `CoughMentionVectors.json`; used by `ClinicalTextParser` and the PANE mapper). The patient questionnaires (iPad `EncounterQuestionnaire`, web intake, APCQ) pick pain or lump questions from the same symptom type. Registered as `history-frames` (unreviewed; sign-off lists in `docs/clinical-validation/changes/history-by-complaint.md` and `followups-history.md`).
...
- **The disease-centred vademecum is phase 1, SHADOW ONLY (nothing in production reads it).** Content `clinical-content/vademecum/{findings,abdominal-pain,cough-breathlessness}.json` (schemas `clinical-content/schemas/vademecum-*.schema.json`; iOS bundles the `vademecum` folder and only decodes it, `VademecumContent.swift`); loop `lib/pane-engine/src/vademecum-loop/` (import `@workspace/pane-engine/vademecum-loop`, not from the package root). `lint:shared-content` checks schemas / Swift / TS types / registry for both folders; `lint:vademecum` fails on a broken reference (finding, exam sign, rule band, analyte, PANE id, iOS candidate, decision, history frame), malformed criteria logic, or anything marked reviewed. Compare with the current engines: `pnpm --filter @workspace/scripts run clinval:web -- --engine vademecum` (writes `vademecum-shadow-latest.*`, which is ignored by version control; never blocks). Every value is `fromMemory` and pending sign-off (`docs/clinical-validation/changes/vademecum-phase1.md`); a content change bumps the file `version` with its registry entry. Don't wire it into a production panel before the switching criteria in `docs/VADEMECUM-PLAN.md` hold and the surgeon has signed the values off.

- **Reference ranges have one source, and the lab feed files only exact matches (Migration 96).** Every normal range, ULN and critical limit is read through `@workspace/triage-engine/reference-ranges` (practice rows from `lab_reference_ranges`, edited by an admin in Settings, else the built-in adult defaults, which reproduce the numbers the app used before); the iOS read-only twin `LabReferenceRanges.swift` is CI-checked by `lint:reference-range-parity` — change both, and the Migration 96 seed, together. Don't add a hard-coded range or "× ULN" to a consumer; pass the practice ranges in. The decision layer gets a `uln` only from a practice range (a default keeps the engine's "assumed" wording). The lab feed (`routes/lab-feed.ts`, `lib/lab-feed/*`) attaches a result only on exact MRN **and** date of birth; everything else goes to `lab_results_to_reconcile` for a nurse / doctor / admin; flags never downgrade the laboratory's; message bodies are never stored or logged; critical alerts are in-app plus a PHI-free staff email through `sendOrDraft`. See `docs/LAB-FEED.md` and `docs/clinical-validation/changes/lab-feed.md`.

- **The real-outcomes loop (Migration 94) records codes only and never changes an engine.** Web completion sends a prediction snapshot with `POST /api/visit/complete` (`lib/prediction-snapshots.ts`, first completion wins); clinicians confirm final diagnoses on the patient summary (`components/outcomes/FinalDiagnosisPanel.tsx`); the admin "Engine accuracy" page and `pnpm --filter @workspace/scripts run outcomes:calibration` report accuracy and write *proposals* only. Shared maths and the sanitisers that must match the migration's CHECKs live in `@workspace/triage-engine/outcomes`. iOS keeps the same JSON on `Encounter.predictionSnapshotJson` / `finalDiagnosisJson` and pushes / pulls it (`SyncService+Outcomes.swift`, nurse / doctor / admin only) through `OutcomeSanitiser.swift`, the Swift twin of the sanitisers (shared vectors `ios/AmiseMedFlowTests/Resources/OutcomeSanitiserVectors.json`, run by both platforms). Every client tolerates the tables being absent (iOS: skipped quietly, retried six hours later or at sign-in). See `docs/clinical-validation/changes/outcomes-calibration.md`.

- **Approved-content channel (Migration 98): a signed-off rule file can reach web and iOS without an app update, and the bundled file is always the fallback.** Releases live in `clinical_content_releases` (append-only except a one-time revocation; staff read, doctor/admin publish as themselves) and are written only by `pnpm --filter @workspace/scripts run content:publish -- <registry id> --signoff-ref … --publisher-email …` (prints SQL by default; refuses while the registry records no clinical review; never sets `lastReviewed`). Every client re-verifies each release with `@workspace/triage-engine/approved-content` / `ApprovedContent.swift` (canonical-JSON SHA-256, id, version strictly above bundled, this build's schema via a fail-closed schema-subset checker pinned to ajv, and only the top-level keys in `APPROVED_CONTENT_POLICY` may differ) — any failure or a missing table → bundled. Enabled for `diagnostic-reasoning-zebras` (`rules`) and `supplement-catalogue` (`items` only: patient wording, prompts and trigger words stay bundled and CI-linted). Content id = registry id = the file's `id`. Shared vectors `ios/AmiseMedFlowTests/Resources/ApprovedContentVectors.json` (`gen:approved-content-vectors`). Change the policy on both platforms together (the parity test reads the Swift). A consumer reads `activeBody(contentId)` at call time (`artifacts/dashboard/src/lib/approved-content-store.ts`); iOS applies it in `SharedClinicalContent.decode` from the next launch. iOS bundles `clinical-content/schemas` as the `schemas` folder. See `docs/APPROVED-CONTENT-CHANNEL.md`.
