...
- **The diagnostic-reasoning layer is a web/iOS twin with shared vectors.** Web: `lib/triage-engine/src/diagnostic-reasoning/*` (core rules, zebra rule set, longitudinal patterns) + `artifacts/dashboard/src/lib/diagnostic-reasoning.ts` (PANE adapter) → `DiagnosticReasoningPanel` on the Assessment step. iOS: `DiagnosticReasoningCore.swift`, `ZebraCheck.swift` (rules: the shared `clinical-content/rules/zebra-rules.json`, the same file the web reads), `LongitudinalPatterns.swift`, `DiagnosticReasoningAdapter.swift` → `DiagnosticReasoningCard.swift` on the Diagnosis step. Both run `ios/AmiseMedFlowTests/Resources/DiagnosticReasoningVectors.json`; `scripts/src/diagnostic-reasoning-parity.test.ts` fails if the thresholds, cost terms or checklist drift (the zebra rules are one shared file, checked by `lint:shared-content`). Deterministic, reads the engines' own evidence (never changes a weight), adds nothing to the record without a tap. Registered as `diagnostic-reasoning-rules` and `diagnostic-reasoning-zebras` (unreviewed; sign-off list in `docs/clinical-validation/changes/diagnostic-reasoning.md`).
