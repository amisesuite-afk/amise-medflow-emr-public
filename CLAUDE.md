...
- **The "What's missing" strip is a web/iOS twin with shared vectors.** Core `lib/pane-engine/src/whats-missing/*` (rules `whats-missing-rules.json`, byte-identical iOS copy `Resources/WhatsMissingRules.json`) and `ios/AmiseMedFlow/Services/WhatsMissingCore*.swift`; adapters `artifacts/dashboard/src/lib/whats-missing-web.ts` (→ `WhatsMissingStrip` under the patient header) and `WhatsMissingPatient.swift` (→ `WhatsMissingRow` under the allergy status). `scripts/src/whats-missing-parity.test.ts` fails when the JSON copies differ or the vectors are stale (`pnpm --filter @workspace/scripts run gen:whats-missing-vectors`). Ranking is lexicographic: safety basics → decision (adverse-probe band flip, risk score, NG12 ferritin, discriminator while unconfirmed) → score completeness. The same record fill pre-fills the Scales-step calculators ("from record") and the iOS populators (`mergeRecord`). A new score input or safety basic goes in the rules JSON, both cores and the vectors in the same PR. Nothing is ordered or recorded without a tap. Registered as `whats-missing` (unreviewed; sign-off list in `docs/clinical-validation/changes/whats-missing.md`).
